...
def get_image_azure():
    ...
    results = requests.post(endpoint, headers = {'Prediction-Key': prediction_key, "Content-Type": "application/octet-stream"}, data = open('/Users/marc.mezger/data/EON/up2date/alle/img15.jpg', 'rb'))
    results = results.json()
    
    res = []
    for prediction in results["predictions"]:
        res.append((prediction["tagName"], prediction["probability"]*100, prediction["boundingBox"]["left"], prediction["boundingBox"]["top"], prediction["boundingBox"]["width"], prediction["boundingBox"]["height"]))
...
def draw_azure_results(res, img,x,y):
    ...
    for detect in res:
        ...
        if conf_score > 60:
            ...
            print('{}: [{}, {}, {}, {}], {}'.format(label, round(topleft_x, 3),
                                                    round(topleft_y, 3), round(bottomright_x, 3),
                                                    round(bottomright_y, 3), round(conf_score, 3)))
